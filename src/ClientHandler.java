...
public class ClientHandler {
        ...
        try {
            ...
            while (isRunning) {
                try {
                    ...
                    Path dir = folderWatcher.getPath(key); //full path zum ordner wo erstellt
                    ...
                    for (WatchEvent<?> event : key.pollEvents()) {
                        ...
                        Path name = ev.context(); //name des ordners
                        Path filename = dir.resolve(name); //full path mit ordner
                        Path relPath = FileManager.getRelPath(filename);
                        ...
                        if (kind == ENTRY_CREATE) {
                            ...
                            if (file.isDirectory()) {
                                FolderInfo info = new FolderInfo(relPath.toString());
                                info.setAdded(true);
                                folderChanges.add(info);
                            } else {
                                FileInfo info = new FileInfo(relPath.toString(), FileManager.getHash(file));
                                info.setAdded(true);
                        ...
                        if (folderWatcher.isDirectoryRegisterd(filename)) {
                            ...
                            FileInfo info = new FileInfo(relPath.toString(), null);
                            info.setRemoved(true);
    ...
    /**
     *
     * @param changes a List of all the folder changes (folder created, folder
     * renamed, folder deleted)
